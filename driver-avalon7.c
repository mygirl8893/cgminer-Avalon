...
int opt_avalon7_freqadj_temp = AVA7_MM701_TEMP_FREQADJ;
...
static inline uint32_t adjust_fan(struct avalon7_info *info, int id)
{
	...
	if ((info->mod_type[id] == AVA7_TYPE_MM701) &&
...
static void detect_modules(struct cgpu_info *avalon7)
{
	...
	for (i = 1; i < AVA7_DEFAULT_MODULARS; i++) {
		...
		if (!strncmp((char *)&(info->mm_version[i]), AVA7_MM701_PREFIXSTR, 3)) {
			info->mod_type[i] = AVA7_TYPE_MM701;
			info->miner_count[i] = AVA7_MM701_MINER_CNT;
			info->asic_count[i] = AVA7_MM701_ASIC_CNT;
...
static int polling(struct cgpu_info *avalon7)
{
	...
	for (i = 1; i < AVA7_DEFAULT_MODULARS; i++) {
		...
		if (ret == AVA7_SEND_OK && !decode_err) {
			...
			if (info->mm_dna[i][AVA7_MM_DNA_LEN - 1] != ar.opt) {
			...
			case AVA7_FREQ_PLLADJ_MODE:
				...
				if (opt_avalon7_smart_speed == AVA7_DEFAULT_SMARTSPEED_MODE2) {
					...
					if ((device_tdiff >= AVA7_DEFAULT_FAVG_TIME) ||
						...
						if (tmp > AVA7_MM701_FREQUENCY_MAX)
							tmp = AVA7_MM701_FREQUENCY_MAX;
						for (j = 0; j < info->miner_count[i]; j++)
