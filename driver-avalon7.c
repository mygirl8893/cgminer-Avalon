...
static void avalon7_set_factory_info(struct cgpu_info *avalon7, int addr, int8_t value)
{
	struct avalon7_info *info = avalon7->device_data;
	struct avalon7_pkg send_pkg;
	uint32_t tmp;
	...
	send_pkg.data[0] = (uint8_t)value;

	applog(LOG_DEBUG, "%s-%d-%d: avalon7 set factory info %d-%d",
			avalon7->drv->name, avalon7->device_id, addr, i, value);
...
static struct api_data *avalon7_api_stats(struct cgpu_info *avalon7)
{
	...
	for (i = 1; i < AVA7_DEFAULT_MODULARS; i++) {
		...
		if (opt_debug) {
			sprintf(buf, " FAC0[%d]", info->factory_info_0);
			strcat(statbuf, buf);
...
char *set_avalon7_device_factory_info(struct cgpu_info *avalon7, char *arg)
{
	struct avalon7_info *info = avalon7->device_data;
	int val, i, j;
	...
	sscanf(arg, "%d", &val);
	applog(LOG_NOTICE, "Update factory voltage offset %d", val);
	if (!val)
		val = AVA7_DEFAULT_FACTORY_INFO_0;

	if (val < AVA7_DEFAULT_FACTORY_INFO_0_MIN || val > AVA7_DEFAULT_FACTORY_INFO_0_MAX)
		return "Invalid value passed to set_avalon7_device_factory_info";

	info->factory_info_0 = val;
	avalon7_set_factory_info(avalon7, 0, info->factory_info_0);

	applog(LOG_NOTICE, "%s-%d: Update factory voltage offset %d",
		avalon7->drv->name, avalon7->device_id, val);
...
static char *avalon7_set_device(struct cgpu_info *avalon7, char *option, char *setting, char *replybuf)
{
	...
	if (strcasecmp(option, "factory") == 0) {
		...
		return set_avalon7_device_factory_info(avalon7, setting);
	}
